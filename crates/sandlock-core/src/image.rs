...
pub fn extract(image: &str, cache_dir: Option<&Path>) -> Result<PathBuf, SandlockError> {
    ...
    let output = Command::new("docker")
        ...
        .output()
        .map_err(|e| SandboxProcessError::Child(format!("docker not found: {}", e)))?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(SandboxProcessError::Child(
...
fn extract_container(container_id: &str, rootfs: &Path) -> Result<(), SandlockError> {
    std::fs::create_dir_all(rootfs)
        .map_err(|e| SandboxProcessError::Io(e))?;
    ...
    let mut child = Command::new("docker")
        ...
        .spawn()
        .map_err(|e| SandboxProcessError::Child(format!("docker export: {}", e)))?;
    ...
    let tar_status = Command::new("tar")
        ...
        .stderr(std::process::Stdio::piped())
        .status()
        .map_err(|e| SandboxProcessError::Child(format!("tar extract: {}", e)))?;

    let docker_status = child.wait()
        .map_err(|e| SandboxProcessError::Child(format!("docker export wait: {}", e)))?;

    if !docker_status.success() {
        // Clean up partial extraction
        let _ = std::fs::remove_dir_all(rootfs);
        return Err(SandboxProcessError::Child("docker export failed".into()).into());
    }

    if !tar_status.success() {
        let _ = std::fs::remove_dir_all(rootfs);
        return Err(SandboxProcessError::Child("tar extraction failed".into()).into());
    }
...
pub fn inspect_cmd(image: &str) -> Result<Vec<String>, SandlockError> {
    let output = Command::new("docker")
        ...
        .output()
        .map_err(|_| SandboxProcessError::Child("docker inspect failed".into()))?;
